...
E2 <- read.table("E2Reg.Timecourse-edgeR.tsv",header=T)
TYPE <- rep("NA", NROW(E2))
...
## Write it out without the "_withGene" component
AN <- cbind(E2, TYPE= as.character(TYPE), CLASS= as.character(CLASS))
write.table(AN, "E2Reg.Transcription.Annotated.tsv", sep="\t", row.names=FALSE, col.names=T, quote=FALSE)
...
write.table(AN, "E2Reg.RNA_Gene.Transcription.Annotated.tsv", sep="\t", row.names=FALSE, col.names=T, quote=FALSE)
