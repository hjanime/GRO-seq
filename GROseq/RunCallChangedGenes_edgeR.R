...
edgeRFindChangedGenes <- function(RefSeqNH00,nrNH00,
					...
					pvalThresh=20.01, expCounts=15000000, G=Gbed, debug=TRUE) {
...
#       print(NROW(unique(as.character(rownames(d$counts)))))
	...
	if(!is.null(FILENAME)) {
                png(paste(FILENAME,".rep.png", sep=""), height=750, width=750)
...
ChangedGenes <- edgeRFindChangedGenes(RefSeqNH00,nrNH00,
					RefSeqLC00,nrLC00, 
					RefSeqNH10,nrNH10, 
					RefSeqLC10,nrLC10, FILENAME="ModelFit-10m.png", G=Gbed)
...
write.table(ChangedGenes, row.names=FALSE, quote=FALSE, file= "E2.10M.Changed_1-13kb.RefSeq.tsv", sep="\t")
...
ChangedGenes <- edgeRFindChangedGenes(RefSeqNH00,nrNH00,
					RefSeqLC00,nrLC00, 
					RefSeqNH40,nrNH40, 
					RefSeqLC40,nrLC40, FILENAME="ModelFit-40m.png", G=Gbed)

#print("Number of Unique Genes 40 at Min")
print(paste("up:",NROW(unique(ChangedGenes$MGI[which(ChangedGenes$FC > 1)]))))
print(paste("dn:",NROW(unique(ChangedGenes$MGI[which(ChangedGenes$FC < 1)]))))

write.table(ChangedGenes, row.names=FALSE, quote=FALSE, file= "E2.40M.Changed_1-13kb.RefSeq.tsv", sep="\t")
...
ChangedGenes <- edgeRFindChangedGenes(RefSeqNH00,nrNH00,
					RefSeqLC00,nrLC00, 
					RefSeqNH160,nrNH160, 
					RefSeqLC160,nrLC160, FILENAME="ModelFit-160m.png", G=Gbed)

print("Number of Unique Genes 160 at Min")
print(paste("up:",NROW(unique(ChangedGenes$MGI[which(ChangedGenes$FC > 1)]))))
print(paste("dn:",NROW(unique(ChangedGenes$MGI[which(ChangedGenes$FC < 1)]))))

write.table(ChangedGenes, row.names=FALSE, quote=FALSE, file= "E2.160M.Changed_1-13kb.RefSeq.tsv", sep="\t")

#source("RunTimecourse_edgeR.R")
######################################################################################################################
######################################################################################################################
